...
class ClassIn {
	...
	LinkedList<Method> methodl = new LinkedList<Method>();
...
class Method {
	...
	int varc = 0;
	LinkedList<Var> varl = new LinkedList<Var>();
	LinkedList<String> formaltl  = new LinkedList<String>();
	LinkedList<String> formalnl  = new LinkedList<String>();
...
public class MJVisitor extends GJDepthFirst<String, String> {
	LinkedList<String> declaredClassList = new LinkedList<String>();			/*Declared classes*/
	LinkedList<Var> allReachableVarsList = new LinkedList<Var>();
	LinkedList<ClassIn> classl = new LinkedList<ClassIn>();
	LinkedList<LinkedList<String>> formalsToBeChecked = new LinkedList<LinkedList<String>>();
	int typeOfLastDeclaredType = -1;											/*Type of the last declared item*/
	...
	String lastTypeName;
	int varc = 0;
	int choiceOfLastStatement = -1;												/*Last picked statement*/
	int typeOfLastExpression = -1;												/*Last expression type*/
	int currentClassNo = -1;
	ClassIn currentClassPtr = null;
	Method currentMethod = null;
	int choiceOfLastPrimaryExpression = -1;
	...
	public boolean checkSameTypes(LinkedList<String> a, LinkedList<String> b) {
		...
		for (int counter = 0; counter < a.size(); counter++) {
			if (!a.get(counter).equals(b.get(counter))) {
	...
	public String visit(ClassDeclaration n, String args) {
		if (FirstPass) {
			if (declaredClassList.contains(n.f1.f0.tokenImage) || n.f1.f0.tokenImage.equals(args)) {
				er("Double declaration of class.\n");
			}
			else {
	...
	public String visit(ClassExtendsDeclaration n, String args) {
		if (FirstPass) {
			if (declaredClassList.contains(n.f3.f0.tokenImage)) {
				...
				er("Trying to extend class that does not exist.\n");
			}
		}
		else if (ThirdPass) {
			if (currentClassNo >= 0)
			...
			if (!inheritPrevBaseClass(currentClassPtr)) {
				er("Unable to inherit super class variables and methods.\n");
			}
				...
				if (temp != null) {
					if (args.equals("SubClassVarDeclaration")) {
						if (!tryOverwriteInheritedVar(temp, mname, mtype))
							er("Variable name declared more than once or trying to overwrite inherited variable with different type variable.\n");
					}	
					else {
						if (verbose)
							System.out.println("Class: " + currentClassPtr.name + ", method: " + currentMethod.name + ", var: " + mname + ".\n");
						er("Variable name declared more than once.\n");
					}
				}
				else {
					if(verbose)
		...
		if(FirstPass) {
			...
			if (mtype != null) {
				...
				if (searchMethodName(classl.peek().methodl, mname) == null && !mname.equals("main")) {		//peek or getlast opws to ha????/
					...
					er("Method name already exists for this class.");
				}
			}
			else {
				er("Method type not correctly declared.\n");
			}
		}
		else if (ThirdPass) {
			...
			if (n.f10.accept(this, null) != currentMethod.type)
				er("Method return type is not the same as the method type.\n");
			...
			if (!currentClassPtr.isBase) {
				...
				if (pmeth != null)
					if (!formalsAreSame(pmeth, currentMethod))
						er("Derived class' method cannot have same name with different parameters.\n");
			}
		...
		if (tempV != null) {
			...
			if (!checkSameTypes(tll1, tll2)) {
				...
				er("Assignment types do not match (" + leftOperanType + " - " + 
						rightOperandType + "). Identifier: " + leftOperand + ".\n");
			}
			else {
				//add code if needed				
			}
			...
			er("Trying to assign value to non declared variable: " + n.f0.f0.tokenImage + "\n");
		}
			...
			if (cuidType != null && cuidType.equals("int[]")) {
				...
				if (exp1Type != null && exp1Type.equals("int")) {
					...
					if (exp2Type == null || !exp2Type.equals("int")) {
						er("Trying to assign non integer expression to int array.\n");
					}
//					else {
//						
//					}
					...
					er("Array index should only be an integer.\n");
				}
			}
			else {
				er("Array assignment should be used only on int arrays. Variable \"" + cuid + "\" is: " + cuidType + ".\n");
			}
		}
		else {
			er("Trying to assign value to non declared variable\n");
		}
		choiceOfLastStatement = -1;
		return null;
	}
	...
	public String visit(IfStatement n, String args) {
		...
		if (exp1Type == null || !exp1Type.equals("boolean")) {
			er("Expression in if evaluation is not boolean.\n");
		}
		n.f4.accept(this, null);
		n.f6.accept(this, null);
		choiceOfLastStatement = -1;
	...
	public String visit(WhileStatement n, String args) {
		...
		String exp1Type = n.f2.accept(this, null);
		if (exp1Type == null || !exp1Type.equals("boolean")) {
			er("Expression in while evaluation is not boolean.\n");
		}
		n.f4.accept(this, null);
		choiceOfLastStatement = -1;
		return null;
	...
	public String visit(PrintStatement n, String args) {
		...
		if (expType == null || !expType.equals("int")) {
			er("Cannot print non int type: " + expType + ".\n");
		}
		choiceOfLastStatement = -1;
		return null;
	}
	...
	public String visit(AndExpression n, String args) {
		...
		if (cl1Type == null || cl2Type == null || !(cl1Type.equals("boolean") && cl2Type.equals("boolean"))) {
			er("Expressions before and after \"&&\" should be boolean.\n");
		}
	...
	public String visit(CompareExpression n, String args) {
		...
		if (pe1Type == null || pe2Type == null || !(pe1Type.equals("int") && pe2Type.equals("int"))) {
			er("Expressions before and after \"<\" should be integers.\n");
		}
		typeOfLastExpression = -1;
		return "boolean";
	}
	...
	public String visit(ArrayLookup n, String args) {
		...
		String pe2Type = n.f2.accept(this, null);
		if (pe1Type == null || pe2Type == null || !(pe1Type.equals("int[]") && pe2Type.equals("int"))) {
			er("Array lookup only to be used on int[] variables with index of int type.\n");
		}
		typeOfLastExpression = -1;
		return "int";
	}
	...
	public String visit(ArrayLength n, String args) {
		...
		if (pe1Type == null || !pe1Type.equals("int[]")) {
			er("Array length only to be used on int[] variables.\n");
		}
		typeOfLastExpression = -1;
		return "int";
	}
				...
				if (mtemp != null) {
					...
					if (mtemp.formaltl.size() == formalsToBeChecked.peek().size()) {
						if(checkSameTypes(mtemp.formaltl, formalsToBeChecked.peek())) {
							...
							er("Method \"" + mtemp.name + "\" takes different type of arguments.\n");
						}
					}
					else {
						er("Method " + mtemp.name + " takes different number of arguments (takes: " + 
								mtemp.formaltl.size() + ", has: " + formalsToBeChecked.peek().size() + ").\n");
					}
					
					formalsToBeChecked.pop();
				}
				else {
					er("This class type does not have such a method.\n");
				}
			}
			else {
				er("Calling member of non declared class or class instance \"" + peType + "\".\n");
			}
		}
		else {
			er("Type of field member caller could not be found.\n");
		}
		return null;
	}
	...
	public String visit(Identifier n, String args) {
		...
		if (args == null) {														/*Classes name declaration*/
			if (FirstPass) {
				...
				if (typeOfLastDeclaredType == 3) {
					if (!declaredClassList.contains(n.f0.tokenImage)) {
						er("Class \"" + n.f0.tokenImage + "\" not declared.\n");
						return null;
					}
					else {
						return n.f0.tokenImage;
			...
			if (isDeclared(n.f0.tokenImage, myClass.varl) != null) {
				er("Type instance name already in use.\n");
			}
			else {
				Var temp;
				if(verbose)
				...
				if (typeOfLastDeclaredType == 0) {
					...
					er("Something weird happened with some type declaration.\n");
				}
			...
			if (typeOfLastDeclaredType == 3) {
				if (!declaredClassList.contains(n.f0.tokenImage)) {
					er("Cannot make an instance of a non declared type.\n");
				}
				else {
	...
	public String visit(ThisExpression n, String args) {
		if (currentClassPtr == null) {
			er("This expression while not in statements");
			return null;
		}
		else
	...
	public String visit(ArrayAllocationExpression n, String args) {
		...
		if (!n.f3.accept(this, null).equals("int")) {
			er("Size of int[] must be an integer.\n");
		}
		return "int[]";
	}
	...
	public String visit(AllocationExpression n, String args) {
		if (declaredClassList.contains(n.f1.f0.tokenImage)) {
			...
			er("Cannot make instance of not declared type.\n");
			return null;
		}
	}
